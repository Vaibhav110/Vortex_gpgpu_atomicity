...
public:
  ...
  static int8_t generate() {
    // Generate small 8-bit integers (avoid overflow when multiplying)
    return static_cast<int8_t>(rand() % 128 - 64);  // range [-64, 63]
  }
...
int main(int argc, char *argv[]) {
  ...
  uint32_t buf_size = size_sq * sizeof(int32_t);

  uint32_t size_sqi = size * size;
  uint32_t buf_size_i = size_sqi * sizeof(int8_t);
  ...
  RT_CHECK(vx_mem_alloc(device, buf_size_i, VX_MEM_READ, &A_buffer));
  RT_CHECK(vx_mem_address(A_buffer, &kernel_arg.A_addr));
  RT_CHECK(vx_mem_alloc(device, buf_size_i, VX_MEM_READ, &B_buffer));
